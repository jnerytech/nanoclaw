...
registerChannel('gmail', (opts: ChannelOpts) => {
  ...
  // OneCLI stub credentials only work inside containers (proxied). Skip polling
  // channel when stubs detected — Gmail MCP tools in containers still work.
  const tokensRaw = fs.readFileSync(tokensPath, 'utf-8');
  if (tokensRaw.includes('onecli-managed')) {
    logger.info(
      'Gmail: OneCLI-managed credentials detected — inbox polling disabled. Gmail tools available via MCP inside containers.',
    );
    return null;
  }
