...
function buildMounts(
  ...
  // Qwen Vision MCP — binary lives in a local private repo, mounted RO.
...
async function buildContainerArgs(
  ...
  // Direct env injection for MCP tools (bypasses OneCLI — intentional for this install).
  const directEnv = readEnvFile(['TODOIST_API_TOKEN', 'LITELLM_API_KEY', 'TAVILY_API_KEY']);
  ...
  try {
    let effectiveAgent: string | undefined = agentIdentifier;
    if (agentIdentifier) {
      try {
        await onecli.ensureAgent({ name: agentGroup.name, identifier: agentIdentifier });
      } catch {
        // ensureAgent failed (e.g. plan only allows one agent) — use default agent
        effectiveAgent = undefined;
      }
    }
    const onecliApplied = await onecli.applyContainerConfig(args, { addHostMapping: false, agent: effectiveAgent });
    if (onecliApplied) {
