...
async function main(): Promise<void> {
  ...
  // Build MCP servers config: nanoclaw built-in + env-injected tools + any from container.json
  const mcpServers: Record<string, import('./providers/types.js').McpServerConfig> = {
    ...
    ...(fs.existsSync('/home/node/.gmail-mcp/credentials.json')
      ? {
          gmail: {
            command: 'bun',
            args: ['run', `${__dirname}/mcp-tools/gmail-server.ts`],
            env: {},
          },
        }
      : {}),
  ...
  for (const [name, serverConfig] of Object.entries(config.mcpServers)) {
    ...
    log(`Additional MCP server from container.json: ${name}`);
  }
